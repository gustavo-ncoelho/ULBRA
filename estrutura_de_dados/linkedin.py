...
def adicionarContato(nome, perfil_linkedin):
    conn = sqlite3.connect('linkedin_network.db')
    cursor = conn.cursor()
...
def adicionarConexao(contato1_id, contato2_id):
    ...
    try:
        cursor.execute('SELECT * FROM conexoes WHERE (contato1_id = ? AND contato2_id = ?) OR (contato2_id = ? AND contato1_id = ?)', (contato1_id, contato2_id, contato2_id, contato1_id))
...
def menu():
    while True:
        ...
        if opc == '1':
            ...
            perfil_linkedin = input('Perdil do Linkedin: ')
        elif opc == '2':
